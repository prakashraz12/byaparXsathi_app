...
export const shopService = {
  createShop: (shopData: Partial<Shop>, status?: 'created' | 'synced') => {
    return database.write(async () => {
      ...
      const CREATED_AT = shopData?.created_at ? shopData.created_at : Date.now();
      const UPDATED_AT = shopData?.updated_at ? shopData?.updated_at : Date.now();
